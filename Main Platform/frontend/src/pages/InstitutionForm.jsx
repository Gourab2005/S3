    ...
    if ("geolocation" in navigator) {
      navigator.geolocation.getCurrentPosition(
        (position) => {
          setFormData((prev) => ({
            ...prev,
            "School Lat": position.coords.latitude.toFixed(6),
            "School Long": position.coords.longitude.toFixed(6),
          }));
        },
        (error) => {
  ...
  return (
    <div className="max-w-4xl mx-auto mt-10 p-6 bg-white shadow-lg rounded-lg">
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        {/* Display Student-Teacher Ratio (calculated dynamically) */}
        <div className="flex flex-col">
          <label className="text-gray-700 font-medium">Student-Teacher Ratio:</label>
          <div className="mt-1 p-2 border rounded-md bg-gray-100 text-gray-800">
            {formData["Total Teachers"] > 0
              ? (formData["Total Students"] / formData["Total Teachers"]).toFixed(2)
              : "N/A"}
          </div>
        </div>
